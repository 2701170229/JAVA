						...
						<table class="table">
							...
							<c:forEach items="${fileList }" var="userFile">
								<tr>
									...
									<td>
                                        <c:if test="${userFile.isShared==1}">
                                            <button class="btn btn-danger" onclick="cancelShare(${userFile.id })">取消共享</button>
                                        </c:if>
                                        <c:if test="${userFile.isShared==2}">
                                            待审核
                                        </c:if>
                                        <c:if test="${userFile.isShared==3}">
                                           共享失败，原因：[${userFile.sharedReason}]
                                        </c:if>
									</td>
