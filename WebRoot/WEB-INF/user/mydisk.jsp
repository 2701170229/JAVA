...
<script type="text/javascript">
	...
	function quickDel(){
	    ...
	    if(0==members){
            layer.confirm('您还不是平台会员，是否立即充值？', {
                btn: ['确实充值','取消']
            }, function(){
                window.location.href="user?action=topUp";
            }, function(){
                layer.msg('别这么小气嘛，还是要支持一下平台哟', {
                    time: 3000
                });
            });
        }else{
	...
	function selectAll(){
	  window.location.href="<%=path%>/user?action=mydisk";
	}
	function select_all(){
		var check = $("#chk_all");
		var ids = $('input[name="ids"]');
			...
			<div class="col-md-12 graphs">
				<div class="xs">
					...
					<div class="bs-example4" data-example-id="contextual-table">
                        <button class="btn" onclick="selectAll()" style="background-color: cadetblue">查询所有</button>
                        <button class="btn btn-danger" onclick="dele()">删除所选项</button>
						<button class="btn btn-primary" onclick="upload()">上传文件到网盘</button>
						<button class="btn btn-primary" onclick="retrieve()" style="background-color: cadetblue">检索重复文件</button>
						<button class="btn btn-primary" onclick="quickDel()" style="background-color: brown">一键删除</button>
							...
							<c:forEach items="${fileList }" var="userFile">
								<tr>
									...
									<c:choose>
										...
										<c:otherwise>
											<input type="button"  value="申请分享" class="btn btn-primary" onclick="share(${userFile.id })"/>
										</c:otherwise>
									</c:choose> 
									</td>
